exports.handler = async (event) => {
  ...
  try {
    if (type === 'quotes') {
      ...
      url = `${BASE}/markets/quotes?symbols=${encodeURIComponent(symbols)}&greeks=false`;
      ...
      const { symbol } = p;
      if (!symbol) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Missing symbol' }) };
      url = `${BASE}/markets/options/expirations?symbol=${encodeURIComponent(symbol)}&includeAllRoots=true&strikes=false`;
